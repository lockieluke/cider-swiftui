...
class AMAPI {
    ...
    func initStorefront() async -> Bool {
        ...
        if let error = res.error {
            self.logger.error("Failed to fetch storefront: \(error.localizedDescription)")
    ...
    func fetchRecommendations() async -> MediaRecommendationSections {
        ...
        if let error = res.error {
            self.logger.error("Failed to fetch recommendations: \(error.localizedDescription)")
        } else if let data = res.data, let json = try? JSON(data: data) {
    ...
    func fetchPersonalRecommendation() async -> [MediaPlaylist] {
        ...
        if let error = res.error {
            self.logger.error("Failed to fetch personal recommendations: \(error.localizedDescription)")
    ...
    func fetchTracks(id: String, type: MediaType) async throws -> [MediaTrack] {
        ...
        if let error = res.error {
            self.logger.error("Failed to fetch tracks: \(error.localizedDescription)")
            throw error
        } else if let data = res.data, let json = try? JSON(data: data) {
            if location == .catalog {
    ...
    func fetchSong(id: String) async throws -> MediaTrack {
        ...
        if let error = res.error {
            self.logger.error("Failed to fetch song: \(error.localizedDescription)")
            throw error
        } else if let data = res.data, let json = try? JSON(data: data) {
    ...
    func fetchArtist(id: String, params: [FetchArtistParams] = [], extendParams: [FetchArtistExtendParams] = []) async throws -> MediaArtist {
        ...
        if let error = res.error {
            self.logger.error("Failed to fetch artist: \(error.localizedDescription)")
            throw error
    ...
    func fetchPlaylist(id: String) async throws -> MediaPlaylist {
        ...
        if let error = res.error {
            self.logger.error("Failed to fetch playlist: \(error.localizedDescription)")
            throw error
    ...
    func fetchAlbum(id: String) async throws -> MediaItem {
        ...
        if let error = res.error {
            self.logger.error("Failed to fetch album: \(error.localizedDescription)")
            throw error
    ...
    func fetchSearchSuggestions(term: String, kinds: [FetchSearchSuggestionsKinds] = [.terms, .topResults], types: [FetchSearchTypes] = [.artists, .songs, .musicVideos, .albums]) async throws -> SearchSuggestions {
        ...
        if let error = res.error {
            self.logger.error("Failed to fetch search suggestions: \(error.localizedDescription)")
            throw error
        } else if let data = res.data, let json = try? JSON(data: data) {
    ...
    func fetchRating(item: MediaDynamic) async -> MediaRatings {
        ...
        guard
            ...
            if let error = res.error {
                self.logger.error("Failed to fetch ratings: \(error.localizedDescription)")
            }
            return .Neutral
        }
    ...
    func setRating(item: MediaDynamic, rating: MediaRatings) async -> MediaRatings {
        ...
        guard
            ...
            if let error = res.error {
                self.logger.error("Error occurred when setting rating to \(rating): \(error.localizedDescription)")
            }
            return rating
        }
    ...
    func fetchLibraryCatalog(item: MediaDynamic) async -> Bool? {
        ...
        if let error = res.error {
            self.logger.error("Failed to check if \(item.id) is in library: \(error.localizedDescription)")
    ...
    func fetchLibraryItem(item: MediaDynamic) async throws -> MediaDynamic {
        ...
        if let error = res.error {
            self.logger.error("Failed to fetch library \(item.type): \(error.localizedDescription)")
            throw error
        } else if let data = res.data, let json = try? JSON(data: data), let albumData = json["data"].array?.first {
    ...
    func searchRecentlyAdded(query: String, limit: Int = 25) async -> [MediaDynamic] {
        let res = await AMAPI.amSession.request("\(APIEndpoints.AMAPI)/me/library/search", parameters: [
            "limit": limit,
            "platform": "web",
            "include": "catalog",
            "extend": "artistUrl",
        ...
        if let error = res.error {
            self.logger.error("Failed to fetch personal social profile: \(error.localizedDescription)")
        } else if let data = res.data, let json = try? JSON(data: data), let items = json["results"]["library-albums"]["data"].array {
            return items.compactMap { item in
                switch MediaType(rawValue: item["type"].stringValue) {
    ...
    func fetchBrowse() async -> [MediaBrowseData] {
        ...
        if let error = res.error {
            self.logger.error("Failed to fetch browse: \(error.localizedDescription)")
    ...
    func fetchRadio() async -> [MediaRadioData] {
        ...
        if let error = res.error {
            self.logger.error("Failed to fetch radio: \(error.localizedDescription)")
        } else if let data = res.data, let json = try? JSON(data: data), let tabsChildren = json["data"].array?.first?["relationships"]["tabs"]["data"].array?.first?["relationships"]["children"]["data"].array {
    ...
    func fetchPlaylists(folder: String = "p.playlistsroot") async -> [MediaPlaylist] {
        ...
        if let error = res.error {
            self.logger.error("Failed to fetch playlists in folder \(folder): \(error.localizedDescription)")
        } else if let data = res.data, let json = try? JSON(data: data) {
    ...
    func fetchLyrics(id: String) async -> String? {
        ...
        if let error = res.error {
            self.logger.error("Failed to fetch lyrics for \(id): \(error.localizedDescription)")
        } else if let data = res.data, let json = try? JSON(data: data) {
