...
"""
    ...
    init(userToken: String, developerToken: String, config: JSON) {
        let userContentController = WKUserContentController().then {
            guard let jsPath = Bundle.main.executableURL?.deletingLastPathComponent().deletingLastPathComponent().appendingPathComponent("SharedSupport").appendingPathComponent("ciderplaybackagent.js"), let jsScript = try? String(contentsOfFile: jsPath.path, encoding: .utf8) else {
